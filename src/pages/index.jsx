...
function Home() {
    ...
    const UsefulLinks = [
        { id: 0, title: 'Инструкции перед тестом', path: '/' },
    ...
    const SliderContent = [
        {
            ...
            description: 'Сэкономьте время и деньги, отсеяв неблагонадежных людей "с порога".',
        ...
        },
        {
            ...
            description: 'Нейтрализуйте финансовые утечки проверкой на "левый" доход за счет работодателя',
        ...
        },
        {
            ...
            description: 'Увеличьте шансы на победу в суде с заключением специалиста или эксперта п.3 ст. 80 УПК РФ',
    ...
    const StatContent = [
        {
            ...
            value: '15',
            title: 'Лет',
            description: 'С 2007 года помогаем установить истину качественной проверкой на полиграфе.'
        },
        {
            id: 1,
            value: '2',
        ...
        },
        {
            ...
            description: 'На выбор Заказчика: эксперт, эксперт высшей категории и Т.Алфеева.'
        }
    ]
    ...
    const WhyTriumfContent = [
        {
            ...
            description: 'Професссиональные полиграфологи Триумфа знают современные, закрытые от интернета методические приемы, позволяющие нейтрализовать попытку обмануть полиграф.'
        },
        {
            id: 1,
            icon: <FaCheck size={24} />,
            title: ' Соблюдение требований',
            description: 'Если соблюдать все универсальные требования к проверке на полиграфе- быстро не получится, но точность того стоит.'
        },
        {
            id: 2,
            icon: <FaCheck size={24} />,
        ...
        },
        {
            ...
            description: 'Что позволяет быть более эффективным и при проверке на полиграфе.'
        }
    ]
                ...
                <div className='max-w-7xl mx-auto px-4'>
                    <div className='lg:pb-24 xs:pb-12'>
                        <Slider {...SliderOptions}>
                            {SliderContent.map((s) => (
                                <div key={s.id}>
                                    <div className='card h-[520px] lg:card-side rounded-md bg-base-100 border border-base-200'>
                                        <figure className='lg:w-1/3 xs:w-full'>
                                            <img className='lg:w-full xs:h-full' src={s.image} />
                                        </figure>
                                        <div className='card-body gap-4 lg:w-2/3 xs:w-full xs:pl-4 xs:pt-4 pr-4 pb-4'>
                                            <h1 className='card-title lg:text-3xl xs:text-xl font-semibold'>{s.title}</h1>
                                            <p className='text-lg font-medium text-neutral/75'>{s.description}</p>
                ...
                <div className='max-w-7xl mx-auto px-4'>
                    <div className='lg:pb-24 xs:pb-12'>
                        <div className='flex flex-col lg:gap-16 xs:gap-8'>
                            <h2 className='lg:text-4xl xs:text-3xl font-bold'>Триумф в цифрах</h2>
                            <div className='grid lg:grid-cols-3 xs:grid-cols-1 xs:gap-4 lg:gap-8'>
                                {StatContent.map((s) => (
                                    <div
                                        className='w-full bg-base-100 border border-base-200 lg:hover:border-base-200/0 text-base-content xs:p-4 p-8 rounded-md flex flex-col items-center justify-center text-center lg:hover:translate-x-1 lg:hover:-translate-y-1 lg:hover:shadow-xl ease-in-out transition-all transform-gpu'
                                        key={s.id}
                                    >
                                        <div className='flex flex-col gap-2'>
                                            <p className='text-secondary text-7xl xs:text-6xl font-bold'>{s.value}</p>
                                            <p className='text-3xl xs:text-2xl font-semibold'>{s.title}</p>
                                            <p className='text-lg xs:text-sm font-medium text-neutral/75 text-center'>{s.description}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className='lg:pb-24 xs:pb-12'>
                        <div className='flex flex-col lg:gap-16 xs:gap-8'>
                            <div className='w-full flex xs:flex-col items-center justify-between xs:gap-8 gap-16'>
                                <h2 className='lg:text-4xl xs:text-3xl font-bold'>Нам доверяют крупные компании</h2>
                                <p className='lg:text-lg font-medium text-neutral/75 xs:text-center'>
                                    Качественное исследование на полиграфе окупается в десятки раз, именно поэтому наши Партнеры–Заказчики возвращаются к нам и рекомендуют нас своим коллегам.
                                </p>
                            </div>
                            <div className='grid lg:grid-cols-3 xs:grid-cols-2 xs:gap-4 lg:gap-8'>
                                {CompanysContent.map((c) => (
                                    <div
                                        className='w-full bg-base-100 border border-base-200 lg:hover:border-base-200/0 text-base-content p-8 xs:p-4 rounded-md flex flex-col items-center justify-center text-center lg:hover:translate-x-1 lg:hover:-translate-y-1 lg:hover:shadow-xl ease-in-out transition-all transform-gpu'
                                        key={c.id}
                                    >
                                        <img className='lg:h-44 xs:h-24' src={c.image} />
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col lg:gap-16 xs:gap-8'>
                            ...
                            <div className='grid lg:grid-cols-2 xs:grid-cols-1 xs:gap-4 lg:gap-8'>
                                {WhyTriumfContent.map((w) => (
                                    <div
                                        className='w-full bg-base-100 border border-base-200 lg:hover:border-base-200/0 text-base-content xs:p-4 lg:p-8 rounded-md flex flex-col xs:gap-2 lg:gap-4 text-justify lg:hover:translate-x-1 lg:hover:-translate-y-1 lg:hover:shadow-xl ease-in-out transition-all transform-gpu'
                                        key={w.id}
                                    >
                                        <div className='w-full flex items-center justify-between'>
                                            <span className='text-success'>{w.icon}</span>
                                            <h2 className='lg:text-xl xs:text-base xs:text-end xs:font-semibold font-bold'>{w.title}</h2>
                                        </div>
                                        <p className='lg:text-lg xs:text-sm font-medium text-neutral/75'>{w.description}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
...
Home.pageDescription = 'Полиграфолог для физ. и юр.лиц проверка на детекторе лжи при расследованиях, при приеме на работу, а также по адвокатскому запросу.'
